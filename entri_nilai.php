<article>
    ...
    ?>
    <form action="dashboard.php?page=entri&nim=<?php isset($_GET['nim'])?$_GET['nim']:''?>
    &kd_mtk=<?php isset($_GET['kd_mtk'])?$_GET['kd_mtk']:'' ?>" method="get" >
        Filter 
        <input type='hidden' name='page' value='entri' >
        <select name="nim" >
            <option value="all">Nama Mahasiswa</option>
        <?php
        ...
        $result1 = mysqli_query($conn, $query1);
            ...
            <select name="kd_mtk" > 
            <option value="all">Matakuliah</option>
            <?php
        ...
        <tr>
            ...
            <td><a href='dashboard.php?page=editnilai&entri=<?=$nim?>&kd_matkul=<?=$kd_matkul?> title='Edit User'>Edit</a> 
                | 
                <a href="dashboard.php?page=deletenilai&entri=<?=$nim?>&kd_matkul=<?=$kd_matkul?>" title='delete User'
                onclick="return confirm('Apakah anda yakin menghapus data ini?')">Delete</a> </td>
        </tr>
        <?php  } ?>
        </table>
        <a href='dashboard.php?page=nilai&entri=' title='entri nilai'>Entri Nilai Mahasiswa</a>
</article>